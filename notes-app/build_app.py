...
BOOKS = [
    ...
    dict(slug="manipal-viva", shelf="interview", kind="merged", search_cap=40000,
         ...
         blurb="All 141 viva questions with the model answer written word for word, in the order a clinical psychology panel actually walks through them: openers, the Definitions Bank, core definitions, MSE and assessment, psychometrics, psychopathology, therapy schools, ethics, research and the dissertation, and the closing questions. The Definitions Bank is the general-concept section, 29 of them, personality through conformity, each one as a single-line definition with the theorist attached and the follow-up they always push with. Every answer is built to one shape, definition then theorist then example then stop, because that is what gets marked in an oral. Drill mode hides every answer so you can say it out loud first and tap the card to check yourself."),
